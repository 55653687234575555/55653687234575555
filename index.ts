...
function serveRandom(res, user, dir) {
...
app.get('/@:user', (req, res) => {
  ...
  const isBrowser = /mozilla/i.test(req.headers['user-agent'] ?? '')

  if (isBrowser)
    return res.redirect(302, `/@${user}/_.gif`)

  serveRandom(res, user, path.join(assets, `@${user}`))
})
...
app.get('/:folder/:file', (req, res, next) => {
  ...
  if (folder.startsWith('@') || !exts.has(ext)) return next()
...
app.get('/:file', (req, res) => {
  ...
  if (!exts.has(ext)) return res.sendStatus(404)
